...
def gen_run_folder(path_model_id=""):
    ...
    run_paths["path_logs_train"] = os.path.join(
        run_paths["path_model_id"], "logs", "train"
    )
    # run_paths["path_logs_eval"] = os.path.join(
    #     run_paths["path_model_id"], "logs", "eval", "run.log"
    # )
    run_paths["path_ckpts_train"] = os.path.join(
        run_paths["path_model_id"], "ckpts", "train"
    )
    # run_paths["path_ckpts_eval"] = os.path.join(
    #     run_paths["path_model_id"], "ckpts", "eval"
    # )
    run_paths["path_gin"] = os.path.join(
        run_paths["path_model_id"], "config_operative.gin"
    )

    # Create folders
    for k, v in run_paths.items():
        if any([x in k for x in ["path_model", "path_ckpts_train", "path_logs_train"]]):
