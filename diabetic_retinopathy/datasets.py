...
def load(name, data_dir):
    if name == "idrid":
        ...
        train_images_list, val_images_list = train_test_split(train_images_list, test_size=103, random_state=7)

        # make the file to tf.data.Dataset objects
        ds_train = tf.data.Dataset.from_tensor_slices(train_images_list).map(lambda x: preprocess(x, label=True),
                                                                             num_parallel_calls=tf.data.experimental.AUTOTUNE)
        ds_val = tf.data.Dataset.from_tensor_slices(val_images_list).map(lambda x: preprocess(x, label=True),
                                                                         num_parallel_calls=tf.data.experimental.AUTOTUNE)
        ds_test = tf.data.Dataset.from_tensor_slices(test_images_list).map(lambda x: preprocess(x, label=False),
                                                                           num_parallel_calls=tf.data.experimental.AUTOTUNE)
