...
@gin.configurable
def rnn_model(input_shape, num_classes, lstm_units, dense_units, dropout_rate):

    inputs = tf.keras.Input(input_shape)
    out = tf.keras.layers.LSTM(lstm_units, return_sequences=True)(inputs)
...
def lstm_model(num_recurrent_layers, num_fc_layers, num_hidden_units, dropout_rate, learning_rate,
    ...
    for i in range(num_recurrent_layers):
        ...
        if stateful:
            ...
            model.add(LSTM(num_hidden_units, return_sequences=True, stateful=stateful,
                           batch_input_shape=batch_input_shape))
        else:
            # 在非状态保持模式下使用原始的input_shape
            model.add(LSTM(num_hidden_units, return_sequences=True, input_shape=input_shape))
